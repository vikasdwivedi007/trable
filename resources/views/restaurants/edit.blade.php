        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h5>{{__("main.update_restaurant")}}</h5>
                ...
                <div class="card-block">
                    <form id="validation-form123" action="{{ route('restaurants.update', ['restaurant'=>$restaurant->id]) }}" method="POST">
                        @method('PATCH')
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="{{__("main.restaurant_name")}}" class="form-control @error('name') is-invalid @enderror" name="name" placeholder="Restaurant Name" required value="{{$restaurant->name}}">
                                    @error('name')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    @foreach($cities as $city)
                                        <option value="{{$city->id}}" @if($city->id == $restaurant->city_id) selected @endif>{{$city->name}}</option>
                                        @endforeach
                                        </select>
                                        @error('city_id')
                                        <span class="invalid-feedback" role="alert">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="Restaurant-Address"
                                           ...
                                           placeholder="{{__("main.restaurant_address")}}" name="address" required value="{{$restaurant->address}}">
                                    @error('address')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="Restaurant-Phn" class="form-control mob_no @error('phone') is-invalid @enderror" name="phone" 
                                           placeholder="{{__("main.restaurant_phone")}}" required value="{{$restaurant->phone}}">
                                    @error('phone')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="email" id="Restaurant-Email" class="form-control @error('email') is-invalid @enderror" 
                                          name="email" placeholder="{{__("main.restaurant_email")}}"  value="{{$restaurant->email}}">
                                    @error('email')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div id="restrntContactsAdded" class="row">
                            @if($restaurant->contacts->toArray())
                                @foreach($restaurant->contacts->toArray() as $key => $value)
                        ...
                        <div id="FoodMenuAdded" class="row">
                            @if($restaurant->menus->toArray())
                                @foreach($restaurant->menus->toArray() as $key => $value)
                                    <div class="menu-item row">                                        
                                        <input type="hidden" name="menus[{{$key}}][id]" value="{{$value['id']}}">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label>{{__('main.food_menu_name')}}</label>
                                                <input type="text" id="Food-Menu-Name" class="form-control @error("menus.{$key}.name") is-invalid @enderror" name="menus[{{$key}}][name]" placeholder="{{__("main.food_menu_name")}}" @isset($value['name']) value="{{$value['name']}}" @endisset>
                                        ...
                                        <div class="col-md-6 align-self-center">
                                            <div class="form-group">
                                                @if(!$loop->first)
                                                    <a href="#" onclick="$(this).closest('.menu-item').remove();return false;">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <textarea placeholder="{{__("main.menu_items")}}" class="form-control @error("menus.{$key}.items") is-invalid @enderror" name="menus[{{$key}}][items]" rows="7">@isset($value['items']) {{$value['items']}} @endisset</textarea>
                            ...
                            @else
                                <div class="menu-item row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label>{{__('main.food_menu_name')}}</label>
                                            <input type="text" id="Food-Menu-Name" class="form-control" name="menus[0][name]" placeholder="{{__("main.food_menu_name")}}" >
                                        </div>
                                    </div>
                                    <div class="col-md-6 align-self-center">
                                        <div class="form-group">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label>{{__('main.menu_items')}}</label>
                                            <textarea placeholder="{{__("main.menu_items")}}" class="form-control " name="menus[0][items]" rows="7"></textarea>
                                        </div>
                                    </div>
                                    <div class="'col-md-6"></div>
                                    <div class="col-md-3">
...
@section('script_bottom')
    <script>
        ...
        @if($restaurant->contacts->count())
            next_contact = {{$restaurant->contacts->count()}};
            @endif
        var next_menu = 1;
        @if($restaurant->menus->count())
            next_menu = {{$restaurant->menus->count()}};
        @endif
    </script>
@endsection
