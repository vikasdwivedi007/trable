        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h5> {{__("main.update_traffic_voucher")}}</h5>
                ...
                <div class="card-block">
                    <form id="validation-form123" action="{{route('traffic-vouchers.update', ['traffic_voucher'=>$voucher->id])}}" method="POST">
                        @csrf
                        @method('PATCH')
                        <div class="row">
                            <div class="col-md-3">
                                ...
                                <div class="form-group">
                                    <label>{{__("main.issued_by")}}</label>
                                      <input type="text" id="IssuedBy" class="form-control"
                                           name="issued_by" placeholder="{{__("main.issued_by")}}" required value="{{$voucher->issued_by}}">
                                </div>
                            </div>
                        </div>
                        <hr>
                                ...
                                <div class="form-group">
                                    <label>{{__("main.to")}}</label>
                                      <input type="text" id="to" class="form-control"
                                           name="to" placeholder="{{__("main.to")}}" required  value="{{$voucher->to}}">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                        ...
                        <hr>
                        <label>{{__("main.arrival_departure_info")}}</label>
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-4">
                                <div class="form-group">
                                    <textarea class="form-control" name="details" rows="6" required>{{$voucher->details}}</textarea>
                                </div>
                            </div>
                        </div>


                        <button id="save-voucher" type="submit" class="btn btn-primary" disabled>{{__("main.update")}}</button>
                        <button type="button" onclick="window.location.href='{{ route('vouchers.index').'#pills-Traffic-tab' }}';" class="btn btn-outline-primary">{{__("main.cancel")}}</button>
                    </form>
