        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h5> {{__('main.update_department')}}</h5>
                ...
                <div class="card-block">
                    <form id="validation-form123" action="{{route('departments.update', ['department'=>$department->id])}}" method="POST">
                        @method('PATCH')
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                  <label>{{__('main.department_name')}}</label>
                                    <input type="text" id="DepartmentName" class="form-control @error('name') is-invalid @enderror"
                                           name="name" required placeholder="{{__('main.department_name')}}" value="{{$department->name}}">
                                    @error('name')
