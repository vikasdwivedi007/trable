        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h5>{{__('main.update_operating_statement')}}</h5>
                ...
                <div class="card-block">
                    <form id="validation-form123" action="{{route('operating-statements.update', ['operating_statement'=>$operatingStatement->id])}}" method="POST">
                        @method('PATCH')
                        @csrf
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="date" name="date" class="form-control"
                                           placeholder="{{__('main.date')}} *" required value="{{$operatingStatement->toArray()['date']}}">
                                </div>
                            </div>
                        </div>
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="EmployeeDDL"
                                        ...
                                        @foreach($employees as $employee)
                                            <option value="{{$employee->id}}" @if($operatingStatement->emp_id == $employee->id) selected @endif>{{$employee->user->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="JobFileDDL"
                                        ...
                                        @foreach($job_files as $job_file)
                                            <option value="{{$job_file->id}}" @if($operatingStatement->job_id == $job_file->id) selected @endif>{{$job_file->file_no}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
